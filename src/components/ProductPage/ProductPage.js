...
function ProductPage() {
    ...
    const handleTryItOnClick = async (e)=>{
        ...
        if(user){
            ...
            setTimeout(()=>{
                    setTrialRoomImage(`https://192.168.1.34:5000/uploads/1_1605561616_product_10.jpg`)
                    window.scrollTo(0,0)
                ...
                <div ref={productImageContainerRef} className='product-image-container'>
                    {trialRoomImage?
                     (<Carousel itemsToScroll={1} itemsToShow={1}>
                         ...
                         <img className='product-image' src={`https://192.168.1.34:5000/data/test/cloth/${product.image}`}/>
                     </Carousel>):
                     <img className='product-image' src={`https://192.168.1.34:5000/data/test/cloth/${product.image}`}/>
                    }
                </div>
